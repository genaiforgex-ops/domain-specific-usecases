#!/usr/bin/env python3
"""Upload JFPSL template DOCX files to the Vertex AI RAG corpus.

Tracks uploaded files in config/.rag_upload_state.json (gitignored) so re-runs
skip unchanged files.

Usage:
    python backend/scripts/sync_templates_to_rag.py
    python backend/scripts/sync_templates_to_rag.py --dry-run
    python backend/scripts/sync_templates_to_rag.py --force
"""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Sync Templates/*.docx to Vertex RAG corpus.")
    parser.add_argument("--templates-dir", type=Path, default=Path(settings.templates_dir))
    parser.add_argument("--state-file", type=Path, default=Path(settings.rag_upload_state_file))
    ...
    candidates = sorted(p for p in templates_dir.glob("*.docx") if p.stat().st_size > 0)
    if not candidates:
        print("[warn] no non-empty DOCX files found")
        return 0
    ...
    print(f"[info] templates: {templates_dir} ({len(candidates)} files, {len(to_upload)} to upload)")
    ...
    if not to_upload:
        print("[done] all templates already synced")
        return 0
